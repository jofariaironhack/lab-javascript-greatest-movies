...
function howManyMovies(movies) {
  let filteredMovies = movies.filter(function (eachMovie) {
    if (
...
function bestYearAvg(movies) {
  ...
  // a forEach that will iterate over movies and will add the attribute of the year
  movies.forEach((eachMovie) => {
    if (!masterObject[eachMovie.year]) {
      masterObject[eachMovie.year] = [eachMovie]; // to create the array with the movie object if the attribute doesn't exist
    } else {
      masterObject[eachMovie.year].push(eachMovie); // to add the movie to the array if the year attribute and the array already exist
    }
  });
